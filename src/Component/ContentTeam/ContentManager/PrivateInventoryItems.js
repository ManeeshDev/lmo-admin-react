...
const PrivateInventoryItems = (props) => {
  ...
  const fetchImages = (value) => {
    ...
    try {
      const endPoint = `${process.env.REACT_APP_BASE_URL}/${empid}/${userRole}?status=${status}&ctgy=inventory`;
      return axios
        .get(endPoint, {
        ...
        .then((res) => {
          updateLoading(false);
          if (res.data.body === null || res.data.body.key.length < 1) {
            updateImagesList([]);
            updateNoImage(true);
          } else {
...
export default PrivateInventoryItems;
