...
class UploadItemMastercsvList extends React.Component {
  state = {
    ...
    if (this.state.selectdCategory !== "") {
      return this.state.activeFileList.length > 0
        ? this.state.activeFileList.map(async (file, index) => {
            ...
            try {
              const endPoint = `${process.env.REACT_APP_BASE_URL}/${empid}/${userRole}?status=master&ctgy=${this.state.selectdCategory}`;
              const config = {
                headers: {
                  Authorization: token,
                },
...
export default UploadItemMastercsvList;
