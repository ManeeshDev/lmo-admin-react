...
class AMUploadTextFile extends React.Component {
  state = {
        ...
        ? this.state.activeFileList.map(async (file, index) => {
            ...
            try {
              ...
              if (
                ...
                endPoint = `${process.env.REACT_APP_BASE_URL}/${empid}/${userRole}?status=uploaditemformat&csv=${this.state.selectedUser}&ctgy=${this.state.selectdCategory}`;
              }
              ...
              if (upload.status === 200) {
                this.setState({ buttonLoading: false });
                Modal.success({
                  content: "Csv File Uploaded Succesfully",
                  onOk() {
...
export default AMUploadTextFile;
