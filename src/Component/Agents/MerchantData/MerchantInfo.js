...
function MerchantInfo({ mrch_id, backToPage }) {
  ...
  const fetchMerchantsData = async () => {
    ...
    try {
      ...
      const endPoint = `${process.env.REACT_APP_BASE_URL}/agent/${empid}?mrchid=${mrch_id}`;
      ...
      setStatusCode(res.data?.body?.status);
    } catch (err) {
      setLoading(false);
  ...
  const approveMerchant = async () => {
    try {
      const endPoint = `${process.env.REACT_APP_BASE_URL}/agent/${empid}?mrchid=${mrch_id}&status=reject`;
